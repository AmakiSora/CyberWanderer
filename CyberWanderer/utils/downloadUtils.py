...
from CyberWanderer.utils.minioUtils import upload_net_file_to_minio
from CyberWanderer.utils.qiniuUtils import upload_net_file_to_qiniu
...
# 下载资源到本地(you-get)
def download_file_local_by_youget(url, file_name='', folder_name='', proxy=False):
    order = ['you-get']
    # 本地路径
    if folder_name:
        order.append('-o')
...
def auto_get_img(urls, download_method):
    ...
    elif download_method.get('method') == 'minio':
        code, statusInfo = multithreading_list(urls,
                                               upload_net_file_to_minio,
                                               ('',
                                                download_method.get('bucket_name'),
                                                download_method.get('useProxy', False)))
        if code == 0:
            return "无图片上传!"
        elif code == 200:
